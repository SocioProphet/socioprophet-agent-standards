...
6a. **Data-namespace crossing also requires a matching geographic-residency toleration.** The data-namespace carries a `region` taint (`NoExecute`); an operation MUST carry a `region=<jurisdiction>` toleration asserting the data may be processed in the operation's jurisdiction (GDPR Ch. V cross-border transfer). Region tolerations are granted per `(principal, jurisdiction)`, are request-scoped and revocable, and — like tenant tolerations — are never role properties. A crossing that lacks the required region toleration is refused fail-closed.
